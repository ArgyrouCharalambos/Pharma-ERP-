...
export default class SalesController {
  public async index({ view, auth }: HttpContext) {
    ...
    .orderBy('created_at', 'desc').where('userid', Number(auth.user?.idProprietaire))

     produit_de_vente = await ProduitDeVente.query().preload('product').where('userid', Number(auth.user?.idProprietaire))
   SALE = await Sale.query()
      ...
      let critiqueResult = await Product.query().where('userid',Number(auth.user?.idProprietaire)).whereRaw('quantity < alert_seuil').count('* as total')
    ...
    .orderBy('created_at', 'desc').where('userid', Number(auth.user?.id))
     ...
     produit_de_vente = await ProduitDeVente.query().preload('product').where('userid', Number(auth.user?.id))
   SALE = await Sale.query()
      .preload('product')
      .orderBy('created_at', 'desc')
      ...
      let critiqueResult = await Product.query().where('userid',Number(auth.user?.id)).whereRaw('quantity < alert_seuil').count('* as total')
                          const Critique = critiqueResult[0]?.$extras.total || 0
                          const Expire =
                            (
                              await Product.query()
    ...
    if(auth.user?.idProprietaire !== null){
      ...
      if(produits && prixTotal && nombreDeProduit){
        const vente = await Sale.create({
          ...
          userid:auth.user?.idProprietaire,
